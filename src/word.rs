use crate::{editing::{add_modification_char, add_tone_char, get_modification_positions, get_tone_mark_placement, replace_nth_char}, parsing::parse_word, processor::{LetterModification, ToneMark}, util::clean_char};
...
impl Word {
    ...
    pub fn is_emtpy(&self) -> bool {
        self.initial_consonant.is_empty() && self.vowel.is_empty() && self.final_consonant.is_empty()
    }
    ...
    pub fn set(&mut self, raw: String) {
        ...
        self.initial_consonant = word.initial_consonant.chars().map(|c| clean_char(c)).collect();
        self.vowel = word.vowel.chars().map(|c| clean_char(c)).collect();
        self.final_consonant = word.final_consonant.to_string();
    }

    pub fn to_string(&self) -> String {
        let mut result = format!("{}{}{}", self.initial_consonant, self.vowel, self.final_consonant);
